...
2.times do |j|
  ...
  (i..max_bound).each do |x|
    ...
    (i..max_bound).each do |y|
      ...
      min_distance = distances.min_by{ |_key, value| value }

      if (distances.find_all{ |_key, value| value == min_distance.last }).length == 1 && i == 0
      ...
      elsif (distances.find_all{ |_key, value| value == min_distance.last }).length == 1 && i == -max_bound
        index = locations.find_index(min_distance.first)
...
locations.length.times do |i|
  if locations_closest_coordinates[i].length == infinity_tester[i].length
    finite_areas.push(locations_closest_coordinates[i].length)
  end
end
